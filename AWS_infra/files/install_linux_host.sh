#! /bin/bash
#while ! curl -H "KEY:redAPIKeySantander22" -X DELETE http://172.23.6.95:2288/api/rest -d '{"index":"agents","paw":"$agent_paw"}' &>/dev/null; do echo "Curl Fail - `date`" && sleep 1; done
server="http://172.23.6.95:2288";
